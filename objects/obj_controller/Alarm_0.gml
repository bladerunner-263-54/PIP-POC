...
with(obj_playfield_parent) {
	...
	if(!_top_square)
	{
		...
		if(_empty_squares > 0)
		{
			...
			// i = 1 sets a value we can increment as we create each piece.
			// i <= _empty_squares means the loop will stop automatically when i reaches 
			...
			for(var i = 1; i <= _empty_squares; i += 1) {
				...
				// Notice we multiply i, the current iteration of this loop, by -100, the size of the grid squares.
