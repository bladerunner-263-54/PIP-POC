...
function check_board_valid() 
{
	...
	with(obj_matching_piece) {
		...
		for(var i = 0; i < 4; i += 1) {
			...
			// We set this to the current x position, plus lengthdir_x( 100, i*90)
			...
			// We set this to the current y position, plus lengthdir_y( 100, i*90)
			// This will tell each iteration of the for loop to check the squares
			// above, below, left and right of the current piece for matches.
			...
			while(true)
			{
				...
				if(_piece_to_check)
				{
					...
					// i.e. Check if these two objects are matching pieces!
